...
import { useLocation, useHistory, BrowserRouter, Switch, Route } from 'react-router-dom';
import decode$1 from 'jwt-decode';
...
var ConsoleLogHelper = function ConsoleLogHelper() {
  var _console;
...
var LoginCallback = function LoginCallback(_ref) {
  ...
  useEffect(function () {
    var queryString = lib.parse(location.search, {
  ...
  return /*#__PURE__*/React.createElement("div", null, "Login Callback");
};

var _excluded = ["children"];
var globalAuthState = createState({
  authToken: 'empty',
  deviceFP: 'empty',
  refresh: false,
  loadstate: 'start',
...
var useGlobalAuthState = function useGlobalAuthState() {
  return useState(globalAuthState);
};

var getAccessTokenRefreshTime = function getAccessTokenRefreshTime(token) {
  if (token == 'empty') {
...
var decodedAccessToken = function decodedAccessToken(accessToken) {
  if (!accessToken || accessToken === 'empty') return 'empty';
  var decoded;

  try {
    decoded = decode$1(accessToken);
...
var refreshCountState = createState(0);
var UserAuth = function UserAuth(_ref) {
  var children = _ref.children,
      loginUrl = _ref.loginUrl,
      refreshTokenUrl = _ref.refreshTokenUrl,
      LogincallbackUrl = _ref.LogincallbackUrl,
      headersSend = _ref.headersSend;
  var Authstate = useState(globalAuthState);
  ConsoleLogHelper('Loading state:', Authstate.loadstate.get());
  var refreshCount = useState(refreshCountState);

  var onActive = function onActive(e) {
    ConsoleLogHelper("Called onActive");

    if (Authstate.privateRoute.get() === true) {
      var decodedToken = decodedAccessToken(Authstate.authToken.get());

      if (decodedToken === undefined) {
        ConsoleLogHelper('Set Empty for onActivity');
        Authstate.authToken.set('empty');
      } else {
        var x = getAccessTokenRefreshTime(decodedToken);
        ConsoleLogHelper('On activity is refresh needed?:', x);

        if (x === true) {
          ConsoleLogHelper('Refresh count up:', refreshCount.get());
          refreshCount.set(function (p) {
            return p + 1;
          });
        }
      }
  ...
  return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement(IdleTimer, {
    ...
    exact: true
  }, /*#__PURE__*/React.createElement(LoginCallback, {
    Authstate: Authstate
  })), /*#__PURE__*/React.createElement(Route, null, children))));
};

var PrivateRouteChecker = function PrivateRouteChecker(_ref2) {
  var children = _ref2.children;
  ConsoleLogHelper("on private route");
  var auth = useGlobalAuthState();
  var refreshCount = useState(refreshCountState);
  useEffect(function () {}, [auth.privateRoute.get(), auth.authToken.get()]);

  if (auth.privateRoute.get() === false) {
    ConsoleLogHelper("setting private route now");
    auth.privateRoute.set(true);
    refreshCount.set(function (v) {
      return v + 1;
    });
    return null;
  }
...
var PrivateRoute = function PrivateRoute(_ref3) {
  ...
  return /*#__PURE__*/React.createElement(Route, rest, /*#__PURE__*/React.createElement(PrivateRouteChecker, null, children));
};

export { PrivateRoute, UserAuth, useGlobalAuthState };
